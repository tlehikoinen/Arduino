...
void resetArrow(struct st7735 * lcd, uint8_t row, uint16_t color_bckgrn) {
	char arrow[] = "->";
	if (row == 0) 
		ST7735_SetPosition(0,0);
	else if (row == 1) {
		ST7735_SetPosition(0,20);
	} 
	else if (row == 2) {
		ST7735_SetPosition(0,40);
	  }
	else if (row == 3) {
		ST7735_SetPosition(0,60);
	}
	else if (row == 4) {
		ST7735_SetPosition(0,80);
	}
	ST7735_DrawString(lcd, arrow, color_bckgrn, X3);
}
void setArrow(struct st7735 * lcd, uint8_t row, uint16_t color_select) {
	char arrow[] = "->";
	if (row == 0) 
		ST7735_SetPosition(0,0);
	else if (row == 1) {
	...
	ST7735_DrawString(lcd, arrow, color_select, X3);
}

//void initArrow(struct st7735 * lcd) {
	//resetArrow(lcd, 1);
	//setArrow(lcd, 0);
//}

void moveArrow(struct st7735 * lcd, uint8_t previous_row, uint8_t current_row, uint16_t color_bckgrn, uint16_t color_select) {
	resetArrow(lcd, previous_row, color_bckgrn);
	setArrow(lcd, current_row, color_select);
}

void drawSnakeMenu(struct st7735 * lcd, uint8_t row, uint16_t color_bckgrn, uint16_t color_text, uint16_t color_select) {
	//setSpiState(display);
	ST7735_ClearScreen(lcd, color_bckgrn);
	ST7735_SetPosition(20,0);
	ST7735_DrawString(lcd, "Play a game",  color_text, X3);
	ST7735_SetPosition(20,20);
	ST7735_DrawString(lcd, "Highscores",  color_text, X3);
	ST7735_SetPosition(20,40);
	ST7735_DrawString(lcd, "Settings",  color_text, X3);
	setArrow(lcd, row, color_select);
}

void drawHighscoreMenu(struct st7735 * lcd, uint16_t *value, uint8_t length, uint16_t color_bckgrn, uint16_t color_text, uint16_t color_select) {
	ST7735_ClearScreen(lcd, color_bckgrn);
	ST7735_SetPosition(20,0);
	ST7735_DrawString(lcd, "Highscores", color_text, X3);
	ST7735_SetPosition(20,20);
	ST7735_DrawString(lcd, "Reset", color_text, X3);
	setArrow(lcd, 0, color_select);
	
	uint8_t y_pos = 20;
	for (uint8_t i=0;i<length;i++) {
		char charArray[32];
		itoa(value[i],charArray,10);
		ST7735_SetPosition(100,y_pos);
		ST7735_DrawString(lcd, charArray, color_text, X3);
		y_pos+=20;
	}
}

void drawEndScreen(struct st7735 * lcd, char *value, uint8_t newHighscore, uint16_t color_bckgrn, uint16_t color_text) {
	if (newHighscore) {
		ST7735_ClearScreen(lcd, color_bckgrn);
		ST7735_SetPosition(3,10);
		ST7735_DrawString(lcd,"NEW HIGH SCORE", color_text, X3);
		ST7735_SetPosition(70,50);
		ST7735_DrawString(lcd, value, color_text, X3);
	} else {
		ST7735_ClearScreen(lcd, color_bckgrn);
		ST7735_SetPosition(30,10);
		ST7735_DrawString(lcd,"Good game", color_text, X3);
		ST7735_SetPosition(70,50);
		ST7735_DrawString(lcd, value, color_text, X3);
	}
}

void drawSettingsMenu(struct st7735 * lcd, uint16_t color_bckgrn, uint16_t color_text) {
	ST7735_ClearScreen(lcd, color_bckgrn);
	ST7735_SetPosition(20,0);
	ST7735_DrawString(lcd,"SETTINGS", color_text, X3);
	ST7735_SetPosition(20,20);
	ST7735_DrawString(lcd,"Difficulty", color_text, X3);
	ST7735_SetPosition(20,40);
	ST7735_DrawString(lcd,"Color theme", color_text, X3);
}

void drawSpeedMenu(struct st7735 * lcd, uint16_t color_bckgrn, uint16_t color_text) {
	ST7735_ClearScreen(lcd, color_bckgrn);
	ST7735_SetPosition(20,0);
	ST7735_DrawString(lcd,"Change Speed", color_text, X3);
	ST7735_SetPosition(20,20);
	ST7735_DrawString(lcd,"Slow", color_text, X3);
	ST7735_SetPosition(20,40);
	ST7735_DrawString(lcd,"Medium", color_text, X3);
	ST7735_SetPosition(20,60);
	ST7735_DrawString(lcd,"Fast", color_text, X3);
}

void drawColorMenu(struct st7735 * lcd, uint16_t color_bckgrn, uint16_t color_text) {
	ST7735_ClearScreen(lcd, color_bckgrn);
	ST7735_SetPosition(20,0);
	ST7735_DrawString(lcd,"Change theme", color_text, X3);
	ST7735_SetPosition(20,20);
	ST7735_DrawString(lcd,"White", color_text, X3);
	ST7735_SetPosition(20,40);
	ST7735_DrawString(lcd,"Dark", color_text, X3);
	ST7735_SetPosition(20,60);
	ST7735_DrawString(lcd,"Color", color_text, X3);
}
void drawX(struct st7735 * lcd, uint8_t row, uint16_t color) {
	if (row == 0) {
		ST7735_SetPosition(100, 20);
	}
	else if (row == 1) {
		ST7735_SetPosition(100, 40);
	}
	else if (row == 2) {
		ST7735_SetPosition(100, 60);
	}
	ST7735_DrawString(lcd, "X", color, X3);
}

void undrawX(struct st7735 * lcd, uint8_t row, uint16_t color) {
	if (row == 0) {
		ST7735_SetPosition(100, 20);
	}
	else if (row == 1) {
...
void drawSelectedSpeed(struct st7735 * lcd, uint8_t row, uint16_t color_bckgrn, uint16_t color_select) {
	if (row == 0) {
		undrawX(lcd, 1, color_bckgrn);
		undrawX(lcd, 2, color_bckgrn);
		drawX(lcd, 0, color_select);
	}
	else if (row == 1) {
		undrawX(lcd, 0, color_bckgrn);
		undrawX(lcd, 2, color_bckgrn);
		drawX(lcd, 1, color_select);
	}
	else if (row == 2) {
		undrawX(lcd, 0, color_bckgrn);
		undrawX(lcd, 1, color_bckgrn);
		drawX(lcd, 2, color_select);
	}
}